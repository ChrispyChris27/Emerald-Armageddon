...
#define B_DEFOG_EFFECT_CLEARING         GEN_6 // In Gen5+, Defog does not lower the evasion of a target behind a Subsitute. In Gen6+, Defog also clears hazards from the user's side. In Gen8+, Defog also clears active Terrain.
...
#define B_STEAL_WILD_ITEMS          GEN_3 // In Gen9+, Thief and Covet steal a wild Pokémon's item and send it to the Bag. In Gen8 and earlier, the stolen item would be held by the Thief/Covet user.
...
// Other battle flags
#define B_FLAG_INVERSE_BATTLE       FLAG_INVERSE_BATTLES     // If this flag is set, the battle's type chart inverted. For example, Fire is super effective against Water.
#define B_FLAG_AI_VS_AI_BATTLE      0     // If this flag is set, the player's Pokémon will be controlled by the AI in the next battle.
#define B_FLAG_DYNAMAX_BATTLE       FLAG_WILD_DYNAMAX_BATTLE    // If this flag is set, the ability to Dynamax in battle is enabled for all Trainers.
...
#define B_FLAG_NO_WHITEOUT          FLAG_NO_WHITE_OUT     // If this flag is set, the player can not white out against Trainers. Please note that the party is not healed automatically!
...
// Sky Battles
#define B_FLAG_SKY_BATTLE                 FLAG_SKY_BATTLES     // If this flag has a value, the player will be able to engage in scripted Sky Battles.
#define B_VAR_SKY_BATTLE                  VAR_SKY_BATTLE_STATE     // If this var has a value, the game will remember the positions of Pokémon used in Sky Battles.
...
#define B_ABILITY_WEATHER               GEN_3 // In Gen6+, Ability-induced weather lasts 5 turns. In Gen5 and earlier, it lasted until the battle ended or until it was changed by another weather condition.
