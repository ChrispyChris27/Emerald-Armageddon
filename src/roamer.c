...
bool8 TryStartRoamerEncounter(void)
{
    ...
    for (i = 0; i < ROAMER_COUNT; i++)
    {
        if (IsRoamerAt(i, gSaveBlock1Ptr->location.mapGroup, gSaveBlock1Ptr->location.mapNum) == TRUE && (Random() % 1) == 0)
        {
...
{
    TryAddRoamer(SPECIES_ARTICUNO, 80);
    TryAddRoamer(SPECIES_ARTICUNO_GALAR, 80);
    TryAddRoamer(SPECIES_ZAPDOS, 80);
    TryAddRoamer(SPECIES_ZAPDOS_GALAR, 80);
    TryAddRoamer(SPECIES_MOLTRES, 80);
    TryAddRoamer(SPECIES_MOLTRES_GALAR, 80);
    TryAddRoamer(SPECIES_FEZANDIPITI, 80);
