...
static bool32 AI_IsMoveEffectInPlus(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, s32 noOfHitsToKo)
{
    ...
    for (u32 effectIndex = 0; effectIndex < additionalEffectCount; effectIndex++)
    {
        ...
        {
            if (IsAdditionalEffectBlocked(battlerAtk, abilityAtk, battlerDef, abilityDef, move) || CanAIFaintTarget(battlerAtk, battlerDef, 0))
                continue;
...
enum AIPivot ShouldPivot(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move)
{
    ...
    if (gAiLogicData->abilities[battlerAtk] == ABILITY_REGENERATOR && aiIsFaster && ShouldRecover(battlerAtk, battlerDef, move, 33) && hasGoodSwitchin)
        return SHOULD_PIVOT;
    if (gAiLogicData->abilities[battlerAtk] == ABILITY_REGENERATOR && !aiIsFaster && hasGoodSwitchin)
        return SHOULD_PIVOT;
